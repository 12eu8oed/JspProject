	...
	%>	
		...
		<% 
			if(count > 0){
				...
				if(startPage > 10){
		%>				
					<a href="Main.jsp?center=BoardList.jsp?pageNum=<%=startPage-10%>">[이전]</a>
		<%			
				}	 
				//페이징 처리 
				for(int i=startPage;i<=endPage;i++){
		%>			
					<a href="Main.jsp?center=BoardList.jsp?pageNum=<%=i%>">[<%=i %>]</a>
		<% 			
				}
				//다음이라는 링크를 만들건지 파악
				if(endPage < pageCount){
		%>		
				<a href="Main.jsp?center=BoardList.jsp?pageNum=<%=startPage+10%>">[다음]</a>	
		<% 
				}	
			}
		%>
